...
var eo = module.exports = function emailobj(from, to, subject) {
  ...
  for(var i in eo.handlers) {
    var t = new eo.handlers[i](this);
...
require('fs').readdirSync(__dirname + '/headers/').forEach(function(v) {
  if(v[0] == '.') return;
  eo.handlers.push(require('./headers/' + v));
});
var genericHeader  = eo.genericHeader = require('./generic-headers.js');
eo.msgpart = require('./msg-part.js');
...
eo.prototype.compile = function(opts) {
  ropts = opts || {
    asString: false || opts.asString
    ,lineLimit: opts.lineLimit || eo.lineLimitSoft
  };
  ...
  var result = [];
	for(var i in this._headerHandlers) {
		...
		if(require('util').isArray(tmp) && !this._headerHandlers[i].fromRaw) {
			for(var j in tmp) {
				if(tmp[j].length > ropts.lineLimit) {
          var tstr = "", tarr = tmp[j].split(/\x20/);
          if(tarr[0] == "") { tarr.shift(); }
          //var prefix = ["", "  "];
          for(var k in tarr) {
            if(tstr.length + tarr[k].length < (ropts.lineLimit-3)) {
              tstr += (tarr[k] + " ");
            } else {
              result.push(tstr);
              tstr = "  " + tarr[k];
  ...
  if(this.isMultipart) {
	...
	return (ropts.asString) ? result.join("\r\n") : result;
};
eo.parseHeaders = function(rawheaders, msg) {
  ...
  for(var i in ha) {
    ...
    var kv = ha[i].match(headerValueRegex);
...
eo.parse = function(raw) {
  ...
  if(msg.isMultipart) {
    ...
    for(var i in rawparts) {
      var pd = eo.msgpart.parse(rawparts[i]);
      if(typeof pd != 'undefined') msg.data.push(pd);
    }
  } else {
    msg.data.push(new eo.msgpart(msg.headers['content-type'].value, rawbody));
  }
  return msg;
};
